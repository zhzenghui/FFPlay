...
void audioQueueOutputCallback(void *inClientData, AudioQueueRef inAQ,
    ...
    AudioPlayer *audioController = (__bridge AudioPlayer*)inClientData;
...
@property (nonatomic, assign) FFPlayer *streamer;
    ...
    if (buffer) {
        ...
        while (_streamer.audioPacketQueue.count && buffer->mPacketDescriptionCount < buffer->mPacketDescriptionCapacity) {
            ...
            if (buffer->mAudioDataBytesCapacity - buffer ->mAudioDataByteSize >= packet->size ) {
                ...
                memcpy((UInt8 *)buffer->mAudioData +buffer->mAudioDataByteSize, packet->data, packet->size);
        ...
        if (buffer->mPacketDescriptionCount >  0) {
            ...
            if (status != noErr) {
                NSLog(@"could not enqueur bufferl.");
            }
        }
        else {
...
- (OSStatus)startQueue {
    ...
    if (! state_ ) {
        ...
        state_ = AudioQueueStart(audioQueue_, NULL) ;
